...
"%WDHT_PY%" -m clanker_hydrus_tagger search-artist "%~dp0%SEARCH_ARTIST_FILE%" --token "%HYDRUS_TOKEN%" --host "%HYDRUS_HOST%" --tag-service "%TAG_SERVICE%" --privacy %PRIVACY% --timeout %SEARCH_TIMEOUT% --doublecheck-file-system %DOUBLECHECK% --namespace "%SEARCH_ARTIST_NAMESPACE%" --sites "%SEARCH_ARTIST_SITES%"
pause
exit /b %errorlevel%
