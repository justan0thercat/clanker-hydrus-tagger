...
"%WDHT_PY%" -m clanker_hydrus_tagger search-year "%~dp0%SEARCH_YEAR_FILE%" --token "%HYDRUS_TOKEN%" --host "%HYDRUS_HOST%" --tag-service "%TAG_SERVICE%" --privacy %PRIVACY% --timeout %SEARCH_TIMEOUT% --doublecheck-file-system %DOUBLECHECK% --namespace "%SEARCH_YEAR_NAMESPACE%" --sites "%SEARCH_YEAR_SITES%"
pause
exit /b %errorlevel%
